    ...
    <div className="min-h-screen bg-slate-900 flex flex-col">
      ...
      <div className="flex-1 p-3 grid grid-cols-2 gap-2 content-start">
        {photos.map((photo) => {
          ...
          return (
            <div key={photo.id} className="relative aspect-square rounded-xl overflow-hidden bg-slate-800 group cursor-pointer"
              onClick={() => toggleSelect(photo.id)}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={photo.url} alt={photo.filename} className={`w-full h-full object-cover transition-all duration-200 ${isSelected ? "scale-95 brightness-75" : "group-active:scale-95"}`} loading="lazy" />
              {isSelected && (
                <div className="absolute inset-0 flex items-center justify-center bg-[#4B4FAE]/30">
                  <div className="w-8 h-8 bg-[#4B4FAE] rounded-full flex items-center justify-center shadow-lg">
              ...
              <button onClick={(e) => { e.stopPropagation(); setLightbox(photo); }}
                className="absolute top-2 right-2 w-7 h-7 bg-black/60 rounded-full items-center justify-center hidden group-hover:flex">
                <ZoomIn size={14} className="text-white" />
              </button>
              <a href={photo.url} download={photo.filename} target="_blank" onClick={(e) => e.stopPropagation()}
                className="absolute bottom-2 right-2 w-8 h-8 bg-black/70 backdrop-blur-sm rounded-full flex items-center justify-center text-white hover:bg-[#4B4FAE] transition-colors">
                <Download size={14} />
              </a>
            </div>
          );
      ...
      {lightbox && (
        <div className="fixed inset-0 z-50 bg-black/95 flex flex-col" onClick={() => setLightbox(null)}>
          <div className="flex items-center justify-between p-4">
            ...
            <button className="w-9 h-9 bg-white/10 rounded-full flex items-center justify-center text-white"><X size={18} /></button>
          </div>
          <div className="flex-1 flex items-center justify-center p-4">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={lightbox.url} alt={lightbox.filename} className="max-w-full max-h-full object-contain rounded-xl" onClick={(e) => e.stopPropagation()} />
          </div>
          <div className="p-4">
            <a href={lightbox.url} download={lightbox.filename} target="_blank" onClick={(e) => e.stopPropagation()}
              className="flex items-center justify-center gap-2 w-full py-3.5 bg-[#4B4FAE] hover:bg-[#5a5ec0] text-white font-semibold rounded-2xl transition-all">
