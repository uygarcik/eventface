...
import { ArrowLeft, Upload, Cpu, QrCode, ImageIcon, Loader2 } from "lucide-react";
...
export default function EventDetailPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  const tabs = [
    { id: "upload" as Tab, label: t.eventDetail.tabs.upload, icon: Upload },
    { id: "index" as Tab, label: t.eventDetail.tabs.index, icon: Cpu },
    { id: "qr" as Tab, label: t.eventDetail.tabs.qr, icon: QrCode },
  ];
      ...
      <div className="flex gap-1 bg-slate-100 p-1 rounded-xl mb-6">
        {tabs.map(({ id, label, icon: Icon }) => (
          <button key={id} onClick={() => setTab(id)}
            className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-3 rounded-lg text-sm font-medium transition-all ${tab === id ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>
            <Icon size={16} /><span className="hidden sm:inline">{label}</span>
          </button>
        ))}
      </div>

      {tab === "upload" && <PhotoUploader eventId={eventId} onUploaded={fetchEvent} />}
